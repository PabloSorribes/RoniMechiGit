...
public class Shooting : MonoBehaviour
{
	...
	public Transform playerTransform;   //For bulletspawn position
	public Rigidbody playerRigidbody;
	public GameObject projectile;
	public GameObject crosshair;

	private MuzzleFlash muzzleflash;
	private DeflectionHandler deflectionHandler;
	...
	private Vector3 AxisInputDirection()
	{
		...
		if (useEightDirectionAiming)
			axisInput = ClampInputToEightDirectional(axisInput);

		return axisInput.normalized;
	}

	private Vector3 ClampInputToEightDirectional(Vector3 axisInput)
	{
		float deadZone = 0.1f;
		bool isXInDeadzone;
		bool isYInDeadzone;
	...
	private void GetShootInput()
	{
		...
		if (fireCooldownTimer == 0 && deflectionHandler.deflectionState != DeflectionHandler.DeflectionState.deflecting)
		{
			...
			if (CrossPlatformInputManager.GetButtonDown(_Inputs.shootButton + playerController.currentPlayer) || Input.GetKeyDown(KeyCode.Space))
			{
				a_aim.Play();
			}

			//While holding the shoot button, show the Crosshair and stop grounded movement.
			if ((CrossPlatformInputManager.GetButton(_Inputs.shootButton + playerController.currentPlayer) || Input.GetKey(KeyCode.Space)))
			{
			...
			if ((CrossPlatformInputManager.GetButtonUp(_Inputs.shootButton + playerController.currentPlayer) || Input.GetKeyUp(KeyCode.Space)))
			{
	...
	/// <returns></returns>
	private IEnumerator ResetBulletPhysics(Transform p_instantiatedProjectile)
	{
	...
	/// <summary>
	/// Recoil from shot. Different amount of force is applied depending on the player's "grounded"-state.
	/// </summary>
	public void Recoil()
	{
		if (useRecoil)
